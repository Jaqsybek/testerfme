...
async function loadTestsForCategory(category) {
  ...
  try {
    ...
    tests.forEach(test => {
      ...
      option.value = test.filename; // Use safe_filename
      option.textContent = test.display_name; // Use display_name
...
function parseTest(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('<question>')) {
      ...
      if (currentQuestion && currentVariants.length > 0 && correctAnswer) {
        ...
        console.log(`Добавлен вопрос "${currentQuestion}" с ${currentVariants.length} вариантами. Правильный ответ: "${correctAnswer}"`);
      }
  ...
  if (currentQuestion && currentVariants.length > 0 && correctAnswer) {
    ...
    console.log(`Добавлен последний вопрос "${currentQuestion}" с ${currentVariants.length} вариантами. Правильный ответ: "${correctAnswer}"`);
  }
...
async function startAllCategoryTests() {
  try {
    ...
    `;
    allCategoryTests = (await fetchTestList(currentCategory)).map(test => test.filename);
